...
#ttest.results
#rm(ttest.results) #removes the object
#ttest.results #see, it's gone!
#load("big important object.RData")    ##loads it, but note that the name of the object is the same as                                        before, and not the name as the object necessarily. After doing                                        the save, recommendin commenting everything out up until the load                                       (i.e. keep the code there, but dont' run it)
#ttest.results
...
## Weird file types
Package for reading Excel data (contains arguments for which sheet, etc):
  `readxl`  
...
# round() - round it to a specified number of decimal points
x2 = round(x, 1)
x2
# floor() - round down
# ceiling() - round up
...
# sample() - takes a random sample, doesn't have to be with numbers)
# rnorm() - numbers pulled from random distribution
# rbinom() - randomly pulled from a binomial distribution

rbinom(10, size = 10, prob = 0.2) #flip 10 coins, count how many successes i got, and do that ten                                   times (ten runs of ten coin flips)
...
# class() - tells you what class sit is, i.e numeric, character, etc

x = 1:5
x <= 3

#in practice, we use the & and | to join things.

x = 1:6
x > 3
x < 4

(x > 2) & (x < 5) #parentheses not totally necessary here
...
x
#we want to know where the a's and b's are

x == c("a", "b") #not the right way! (checks if the first is a, second is b, third is a, etc)
x %in% c("a", "b") # this is the right way (checks a and b for each one)
